function LF = popovsandu2017_new3_DEnKF2_visual(rmin, rmax, nrs, rsfun, distfun, m, locfun, betamean, D, inflation)
...
if nrs > 5
    optoptions = optimoptions('fminunc', 'Display', 'off', ...
        'UseParallel', true, 'Algorithm', 'quasi-newton', 'FunctionTolerance', eps, 'OptimalityTolerance', eps);
    psopts = optimoptions('fmincon', 'Display', 'off', ...
        'UseParallel', true, 'FunctionTolerance', eps, 'OptimalityTolerance', eps, 'FiniteDifferenceType', 'central');
else
    optoptions = optimoptions('fminunc', 'Display', 'off', ...
        'UseParallel', false, 'Algorithm', 'quasi-newton', 'FunctionTolerance', eps, 'OptimalityTolerance', eps);
    psopts = optimoptions('fmincon', 'Display', 'off', ...
        'UseParallel', false, 'FunctionTolerance', eps, 'OptimalityTolerance', eps, 'FiniteDifferenceType', 'central');
end


%beta = fminunc(Jn, beta, optoptions);

%beta = fmincon(Jn, beta, [], [], [], [], lb, ub, [], psopts);
...
for pass = 1:passes
    for ii = rsperm
        ...
        zz = fminunc(Jnn, beta(ii), optoptions);
...
%     'UseParallel', true);
% beta = patternsearch(Jn, beta, [], [], [], [], [], [], [], psoptions);
...
function J = costfun(HPfHt, locfun, n, beta, rsfun, distfun, t, y, m, d, H, R, bm, Di, Xf, Y, Af, inflation, rmin, rmax)
% fprintf('%.5e\n',beta);
% Y = mean(Ys, 2);
...
if Di == inf
    ...
    if any(beta <= gam) || any(beta > rmax)
        ...
        Ju = ((-sum((aa-1)*log(beta)) + sum((beta)*bb))  );
    end
end
