...
for i = 1:time
    ...
    no = (H*(xt(:, end)) + Rsqrt*randn(numel(obsvars), 1));
    ...
    %modelgood(:, i) = modelmodel.Y0 ;
end


numlayer = 4;
%layers = (numel(obsvars) + nvar)*ones(1, numlayer);
%layers = (round(numel(obsvars)/2))*ones(1, numlayer);
layers = numel(obsvars)*ones(1, numlayer);

net = feedforwardnet(layers);
%netinput = [(obs(:, (spinup + 2):(end - 1)) - H*modelgood(:, (spinup + 2):(end - 1))); modelgood(:, (spinup + 2):(end - 1))];
netinput = (obs(:, (spinup + 2):(end - 1)) - H*modelgood(:, (spinup + 2):(end - 1)));
netoutput = - H*modelgood(:, (spinup + 3):(end));
%netoutput = H*modelgood(:, (spinup + 3):(end));

%netoutput = modelgood(:, (spinup + 2):(end - 1));
...
save('lorenz96bigmodelobsforenetPDA2.mat', 'net');
