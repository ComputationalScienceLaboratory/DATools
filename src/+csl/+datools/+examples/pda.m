...
for L = [8]
    for N = [10]
        for Nchoose = [10]
            ...
            ti    = @(f, t, y) csl.utils.rk4(f, t, y, 1);
            tiadj = @(f, t, y, javp, lambda) csl.utils.rk4adj(f, t, y, javp, lambda);
            ...
            for i = 1:time
                ...
                if size(y, 2) > L
                    ...
                    for ensi = 1:N
                        ...
                        cf = @(yy) cost(yy, L, ti, tiadj, dt, F, Javp);
...
function [c, g] = cost(y, L, ti, tiadj, dt, F, Javp)
...
if nargout > 1
    ...
    for l = 1:(L + 1)
        if l == 1
            ...
            g(:, l) = tiadj(F, TL{l}, YL{l}, Javp, d(:, l));
        elseif l == (L + 1)
            g(:, l) = - d(:, l - 1);
        else
            %g(:, l) = - d(:, l - 1) + (d(:, l) + dt*Javp(0, y(:, l), d(:, l)));
            g(:, l) = tiadj(F, TL{l}, YL{l}, Javp, d(:, l)) - d(:, l - 1);
        end
    end
    g = reshape(g, n*(L + 1), 1);
end
